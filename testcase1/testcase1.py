...
import application_priority_options
###Seagull filenames
...
def set_priority(scenario_path,priority):
    ...
    for elem in tree.iterfind('traffic/send/command/avp[@name="DRMP"]'):
        elem.set('value',str(15-priority))
        logger.info(elem.tag,elem.attrib)
...
def main(*args):
    ...
    ap_option={}
    ap_option=config['DRMP1']['DSRCONFIG']['APPLICATIONPRIORITYOPTIONS']
    ap_option['DSRSO']=testcase.soam
    logger.info(config['DRMP1']['DSRCONFIG']['APPLICATIONPRIORITYOPTIONS'])
    application_priority_options.app_prty_opt_update(**config['DRMP1']['DSRCONFIG']['APPLICATIONPRIORITYOPTIONS'])
    testcase.restart_mps()
    supported_CONN_tests=['IcRateAvg', 'IcRatePeak']
    supported_MP_tests=['MpIcP0Y', 'MpIcP0G','MpIcP7G','MpIcP7Y']
        ...
        if i=='MPTEST':
			...
			while True:
				...
				"""
	                    peg_value=collect_counters(testcase,j)
			    logger.info("Peg value=%s",peg_value)
	                    if peg_value>1000 and peg_value<3005:
			        logger.info("Test pass")
			        break
	                    else:
			        logger.error("Invalid Peg counter")
